...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!payload.client_name || !payload.client_email || !payload.pickup || !payload.dropoff || !payload.datetime || !Number.isFinite(payload.price_eur) || payload.price_eur <= 0) {
      return { statusCode: 400, headers, body: JSON.stringify({ ok:false, error:"Champs requis manquants." }) };
    }

    const db = supa();
    const { data, error } = await db.from("reservations").insert([payload]).select("*").single();
